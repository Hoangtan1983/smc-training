...
-- Quy tắc đúng: hoa hồng = giá gốc (base_price) × tỷ lệ %, phân bổ theo tỷ lệ nộp
-- trên giá sau chiết khấu (final_price), giới hạn 100% khi nộp dư.
...
BEGIN
        ...
        IF v_agent_id IS NOT NULL THEN
            ...
            SET v_base_price = 0; SET v_final_price = 0;
            IF v_invoice_id IS NOT NULL THEN
                SELECT base_price, final_price INTO v_base_price, v_final_price FROM invoices WHERE id = v_invoice_id;
            END IF;
            IF v_final_price IS NULL OR v_final_price <= 0 THEN SET v_final_price = v_base_price; END IF;
            IF v_final_price <= 0 THEN SET v_final_price = v_amount; END IF;
            SET v_commission_amount = ROUND(v_base_price * v_commission_rate / 100 * LEAST(v_amount / v_final_price, 1), 0);
