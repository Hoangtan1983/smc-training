-- SMC Training — Migration v8: sửa sp_approve_payment dùng final_price (giá sau chiết khấu)
-- Đã áp dụng trên production ngày 27/08/2026.
-- Lý do: thủ tục trước đây tính trạng thái hoá đơn theo base_price (giá gốc), trong khi
-- mốc "đóng đủ" đúng phải là final_price (giá sau chiết khấu đại lý). Code PHP đã ghi đè
-- lại nên chưa gây lỗi hiển thị, nhưng đây là bản sửa triệt để.
-- Chỉ đổi một chỗ: base_price -> final_price trong câu UPDATE invoices. Các phần khác
-- (RESIGNAL, fallback agency_id) giữ nguyên theo bản đang chạy trên máy chủ.
