...
export default function TeacherPresentations() {
  ...
  const loadFiles = async () => {
    try {
      const data = await apiGetFiles('presentations');
      setFiles(Array.isArray(data) ? data : []);
    } catch {
      try { setFiles(JSON.parse(localStorage.getItem('smc_teacher_presentations') || '[]')); } catch {}
    }
    setLoading(false);
  };
  ...
  const handleUpload = async (e) => {
    ...
    if (!file) return;
    try {
      await apiUploadFile(file, 'presentations');
      toast.success('Đã upload thuyết trình lên server!');
      await loadFiles();
    } catch (err) {
  ...
  return (
    <div className="animate-fade-in">
      <div className="flex flex-col sm:flex-row items-center justify-between gap-4 mb-6">
        <div><h1 className="text-2xl font-extrabold text-gray-900">Thuyết trình</h1><p className="text-sm text-gray-500 mt-0.5">{files.length} file</p></div>
        <button onClick={() => fileInputRef.current?.click()} className="btn-primary flex items-center gap-2"><Upload className="w-4 h-4" /> Upload thuyết trình</button>
        <input ref={fileInputRef} type="file" accept=".pdf,.ppt,.pptx,.mp4" onChange={handleUpload} className="hidden" />
      </div>
      <div className="relative mb-6"><Search className="w-4 h-4 text-gray-400 absolute left-3 top-1/2 -translate-y-1/2" /><input value={search} onChange={e => setSearch(e.target.value)} className="input-field pl-9" placeholder="Tìm kiếm thuyết trình..." /></div>
      {filtered.length === 0 ? (
        <div className="card p-12 text-center text-gray-400"><FileText className="w-12 h-12 mx-auto mb-3 opacity-30" /><p>Chưa có thuyết trình nào</p></div>
      ) : (
        <div className="space-y-2">{filtered.map(f => (
          <div key={f.id} className="card p-4 flex items-center justify-between"><div className="flex items-center gap-3"><div className="w-10 h-10 rounded-lg bg-amber-50 flex items-center justify-center"><FileText className="w-5 h-5 text-amber-500" /></div><div><div className="font-medium text-gray-900 text-sm">{f.name || f.originalName}</div><div className="text-xs text-gray-400">{(f.size / 1024).toFixed(0)} KB</div></div></div>
