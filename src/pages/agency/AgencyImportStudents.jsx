...
const TEMPLATE_DATA = [
  ['Họ tên', 'Số điện thoại', 'Email', 'Khóa học', 'Học phí', 'Đã nộp', 'Địa chỉ', 'Ghi chú'],
  ['Nguyễn Văn A', '0901000111', 'vana@email.com', 'Hạng A VLOS', '15000000', '5000000', 'Hà Nội', ''],
  ['Trần Thị B', '0901000222', 'thib@email.com', 'Hạng B BVLOS', '20000000', '0', 'TP.HCM', 'Đã tư vấn'],
];

const EMPTY_STUDENT = { fullName: '', phone: '', email: '', course: '', tuition: '', paid: '', address: '', note: '' };
...
export default function AgencyImportStudents() {
  ...
  const handleManualImport = async () => {
    ...
    try {
      ...
      // Convert form data sang format API expects
      const rows = validStudents.map(s => ({
        'Họ tên': s.fullName.trim(),
        'Số điện thoại': s.phone.trim(),
        'Email': s.email.trim(),
        'Khóa học': s.course.trim(),
        'Học phí': s.tuition.trim(),
        'Đã nộp': s.paid.trim(),
        'Địa chỉ': s.address.trim(),
        'Ghi chú': s.note.trim(),
      }));
              ...
              <div className="overflow-x-auto">
                <table className="w-full text-sm">
                  <thead className="bg-slate-50">
                    <tr>
                      {TEMPLATE_DATA[0].map((h, i) => (
                        <th key={i} className="px-4 py-2 text-left font-medium text-slate-600">{h}</th>
                      ))}
                    </tr>
                  </thead>
                  <tbody>
                    {preview.map((row, i) => (
                      <tr key={i} className="border-t border-gray-50 hover:bg-slate-50">
                        {TEMPLATE_DATA[0].map((h, j) => (
                          <td key={j} className="px-4 py-2 text-slate-700">{row[h] || '---'}</td>
                        ))}
                      </tr>
                    ))}
                  </tbody>
              ...
              <table className="w-full text-sm">
                <thead className="bg-slate-50">
                  <tr>
                    ...
                    <th className="px-3 py-2.5 text-left font-medium text-slate-600">Khóa học</th>
                    ...
                    <tr key={index} className={`border-t border-gray-50 ${index % 2 === 0 ? 'bg-white' : 'bg-slate-50/50'}`}>
                      ...
                      <td className="px-3 py-2">
                        <input
                          ...
                          placeholder="VD: Hạng A VLOS, BVLOS"
                          className="w-full border border-gray-200 rounded-lg px-2.5 py-1.5 text-sm focus:outline-none focus:border-orange-400 focus:ring-1 focus:ring-orange-200"
                        />
                      </td>
                      <td className="px-3 py-2">
          ...
          <div className="bg-amber-50 border border-amber-200 rounded-xl p-4 flex items-start gap-3">
            ...
            <div className="text-sm text-amber-700">
              ...
              <ul className="list-disc pl-4 space-y-0.5 text-amber-600">
                ...
                <li>Khóa học có thể để trống (sẽ tự gán khóa mặc định).</li>
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-3 text-sm">
          {[
            ...
            { col: 'Khóa học', required: false, desc: 'VD: "Hạng A VLOS", "Hạng B BVLOS", "A", "B"' },
            { col: 'Học phí', required: false, desc: 'Học phí gốc (VNĐ). Để trống sẽ dùng giá khóa học mặc định.' },
